-- Migration 018: Add deleted_at column for soft-delete support
-- job_postings that have a deleted_at value are considered "deleted"
-- and must be filtered out from all queries.
...
-- Rewrite get_overview_stats to exclude soft-deleted records
...
DECLARE
  ...
  v_activity        JSON;
BEGIN
  -- 1) Totals — only non-deleted records
  ...
  FROM (
    ...
    WHERE user_id = p_user_id AND t.technology <> '' AND deleted_at IS NULL
  ...
  FROM (
    ...
    FROM job_postings
    WHERE user_id = p_user_id AND work_mode <> '' AND deleted_at IS NULL
    GROUP BY work_mode
    ORDER BY COUNT(*) DESC
    LIMIT 3
  ) sub;
  ...
  FROM (
    ...
    FROM job_postings
    WHERE user_id = p_user_id AND seniority <> '' AND deleted_at IS NULL
    GROUP BY seniority
    ORDER BY COUNT(*) DESC
    LIMIT 3
  ) sub;
